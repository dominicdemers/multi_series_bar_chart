        ...
        d3.select(id_name).property("value", start_val).on("change", function () {
            ...
            if(new_TBL_data.length == 0) {
                ...
                d3.select("#no_response_msg")
                  .text("Aucun résultat de sondage trouvé correspondants aux filtres sélectionnés.");
            } else {
                d3.select("#table_div")
                  .style("display","block");
                d3.select("#no_response")
                ...
                if (current_depts.length > 1){
                    ...
                    if (nodata_depts.length > 0) {
                        ...
                        d3.select("#no_response")
                          .style("display","block");
                        d3.select("#no_response_msg")
                          .html("Les organisations suivantes n'ont pas de résultats correspondants aux filtres sélectionnés: " + nodata_string);
                    }
                } 
            }
